    ...
    def __call__(self, inputs):
        if self.training:
            if inputs.ndim == 2:
                ...
                mean = inputs.mean((0,1), keepdim=True)
                var = inputs.var((0,1), keepdim=True)
        else:
...
class FlattenConsecutive:
    ...
    def __call__(self, emb):
        X, T, C = emb.shape
        self.out = emb.view(X, T//self.n, self.n * C)
...
#%%
model = Container([Embedding(27, vector_length), 
                   FlattenConsecutive(2), Layer(2*vector_length, layer_number, bias = False), BatchNorm1D(layer_number), Tanh(), 
                   FlattenConsecutive(2), Layer(2*layer_number, layer_number, bias = False), BatchNorm1D(layer_number), Tanh(), 
                   FlattenConsecutive(2), Layer(2*layer_number, layer_number, bias = False), BatchNorm1D(layer_number), Tanh(), 
                   Layer(layer_number, 27, bias = False)])
# layers = [
...
with torch.no_grad():
    ...
    model.layers[-1].weights *= 0.1 #if batch norm was not used in last layer
...
for layer in model.layers:
    if isinstance(layer, BatchNorm1D):
        layer.training = False
inputs = Xtr
training_logits = model(Xtr)
training_loss = F.cross_entropy(training_logits, Ytr)
for p in model.parameters():
    p.grad = None
